...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        '''
        self.augs_attn = nn.Sequential(Circulant_Linear(in_features=config.hidden_size, out_features=config.hidden_size,
                                                        bias=True, num_cir=4), nn.GELU())
        self.augs_attn2 = nn.Sequential(Circulant_Linear(in_features=config.hidden_size, out_features=config.hidden_size,
                                                         bias=True, num_cir=4), nn.GELU())
        self.augs = nn.Sequential(Circulant_Linear(config.hidden_size, config.hidden_size, bias=True), nn.GELU())
        self.augs2 = nn.Sequential(Circulant_Linear(config.hidden_size, config.hidden_size, bias=True), nn.GELU())
        '''
        self.augs_attn = nn.Sequential(nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size, bias=True),
                                       nn.GELU())
        #self.augs_attn2 = nn.Sequential(nn.Linear(in_features=config.hidden_size, out_features=config.hidden_size, bias=True),
        #                                nn.GELU())
        self.augs = nn.Sequential(nn.Linear(config.hidden_size, config.hidden_size, bias=True), nn.GELU())
        #self.augs2 = nn.Sequential(nn.Linear(config.hidden_size, config.hidden_size, bias=True), nn.GELU())
    ...
    def forward(self, hidden_states, attention_mask, action=1, keep_prob=1.0):
        ...
        if action == 0:
            ...
            #intermediate_input = (hidden_states + attention_output + self.augs_attn(hidden_states) + self.augs_attn2(hidden_states)) * 3.0 / 4
            intermediate_input =attention_output + (hidden_states + self.augs_attn(hidden_states)) / 2.
        ...
        if action == 0:
           ...
           # layer_output = (layer_output + intermediate_input + self.augs(intermediate_input) + self.augs2(intermediate_input)) * 3.0 / 4
            layer_output = layer_output + (intermediate_input + self.augs(intermediate_input)) / 2.
