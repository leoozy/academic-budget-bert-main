...
class AttnConv(nn.Module):
    ...
    def forward(self, x, attention_mask):
        ...
        x = self.selfatten(x, attention_mask)
        return x
...
class DyGraphConv2d(GraphConv2d):
    ...
    def getAttentionMask(self, attention_mask, neighbor_mode):
        #TODO attention for more aggregation operation
        if neighbor_mode == "full":
    ...
    def forward(self, hidden_states, attention_mask, relative_pos=None):
        ...
        if self.neighbor_mode == "full":
            ...
            attention_mask = self.getAttentionMask(attention_mask, self.neighbor_mode)
            hidden_states = super(DyGraphConv2d, self).forward(hidden_states, attention_mask)
...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        self,
        hidden_states_center: torch.Tensor, # B, L, H
        ...
        mixed_query_layer = self.query(hidden_states_center) #  B,L,H

        #CROSS ATTENTION
        key_layer = self.transpose_for_scores(self.key(hidden_states_center)) # B, C, L, H/C
        value_layer = self.transpose_for_scores(self.value(hidden_states_center)) # B, C, L, H/C

        query_layer = self.transpose_for_scores(mixed_query_layer)  # B, C, L, H/C

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) # B,C,L,L
        ...
        context_layer = torch.matmul(attention_probs, value_layer) #B, C, L, H/C
